...
def sqrt(number):
    ...
    # if n is negative then pass
    try:
        if number < 0:
            msg = "\nGiven input {}, so square root of a negative number is undefined.\n".format(number)
...
print ("Pass sqrt(-71) = False"  if (False == sqrt(-71)) else "Fail sqrt(-71)")
...
print ("Pass sqrt(3) = 1024"  if (1024 == sqrt(1048576)) else "Fail sqrt(1048576)")
