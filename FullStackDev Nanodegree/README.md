...
```
pip install SQLAlchemy `OR` pip install flask_sqlalchemy 'OR' pip install -U Flask-SQLAlchemy
...
4. Fill out every `TODO` section throughout the codebase. We suggest going in order of the following:
   - Connect to a database in `config.py`. A project submission that uses a local database connection is fine. When using the project workspace, the default postgres user is <code>postgres</code> and there is no password. The default host is "127.0.0.0" and the default port for database connections is 5432.
...
3. **Verify on the Browser**<br>
   Navigate to the project homepage by clicking the "preview" button in Links menu in the workspace. Note: if you are working locally, use http://127.0.0.1:5000/ or http://localhost:5000.
