...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        # raise NotImplementedError
        return (self.parents[actionA] & set([~i for i in self.children[actionB]]))!=set() or (self.parents[actionB] & set([~i for i in self.children[actionA]]))!=set()
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # TODO: implement this function
        # raise NotImplementedError
        for action_a_pc in actionA.preconditions:
            for action_b_pc in actionB.preconditions:
                if self.parent_layer.is_mutex(action_a_pc, action_b_pc):
                    return True # cond are mutex in parent_layer
        return False 
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        # TODO: implement this function
        # raise NotImplementedError
        for lit_a_act in self.parents[literalA]:
            for lit_b_act in self.parents[literalB]:
                if not self.parent_layer.is_mutex(lit_a_act, lit_b_act):
                    return False
        # if both literals are mutex in self.parent_layer
        return True
...
class PlanningGraph:
    ...
    def find_level_costs(self): # find goals and layer costs
        level_num = 0
        costs = [0]
        goals_finished = set()
        
        for goals in self.goal:
            if goals in self.literal_layers[0]:
                goals_finished.add(goals)
        
        if not self.goal - goals_finished:
            return costs
        ...
        while not self._is_leveled:
            ...
            for goal in self.goal - goals_finished:
                if goal in self.literal_layers[level_num]:
                    costs.append(level_num)
                    goals_finished.add(goal)
    ...
    def h_levelsum(self):
        ...
        # TODO: implement this function
        # raise NotImplementedError
        return (sum(self.find_level_costs()))
    ...
    def h_maxlevel(self):
        ...
        # raise NotImplementedError
        return (max(self.find_level_costs()))
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            finished_goals = True
            extended_layers = self.literal_layers[-1]
            
            for goals in self.goal:
                if goals not in extended_layers:
                    finished_goals = False
                    
            if finished_goals == False:
                level_num += 1
                self._extend()
                continue
            ...
            for a_goal in self.goal:
                for b_goal in self.goal:
                    if extended_layers.is_mutex(a_goal, b_goal):
                        mutex_all = True
